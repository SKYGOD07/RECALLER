    ...
    <Panel title="Borrower and facility" meta={application.id}>
      ...
      {documents?.length ? (
        <ul className="rc-docs">
          {documents.map((d) => (
            <li key={d.id} className="rc-docs__item">
              <span className="rc-docs__type caps">{d.type.replace(/_/g, ' ')}</span>
              <span className="rc-docs__name mono">{d.filename}</span>
              <span className="rc-docs__pages t3 tnum">{num(d.pages)}p</span>
            </li>
          ))}
        </ul>
      ) : null}
