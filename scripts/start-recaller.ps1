...
if (-not $node) {
  ...
  Write-Host ''
  Write-Host '  Then close this window, open a new one, and run this script again.' -ForegroundColor White
...
if ($nodeMajor -lt 20) {
  ...
  Write-Host '  Update from https://nodejs.org/en/download and run this script again.' -ForegroundColor White
  Write-Host ''
  exit 1
}
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    Write-Bad 'The credit engine self-check failed. RECALLER will not start with an unverified engine.'
...
Write-Good 'Credit engine verified'
